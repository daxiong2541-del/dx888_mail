...
export async function getSystemSettings(): Promise<SettingsSnapshot> {
  ...
  const value: SettingsSnapshot = {
    ...
    dynmslWafBypassToken: String((doc as unknown as { dynmslWafBypassToken?: string } | null)?.dynmslWafBypassToken || ''),
  };
...
export async function setDynmslSettings(input: {
  ...
  if (typeof input.dynmslWafBypassToken === 'string') update.dynmslWafBypassToken = input.dynmslWafBypassToken.trim();
