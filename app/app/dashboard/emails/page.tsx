...
export default function EmailsDashboardPage() {
  ...
  const [dynmslToken, setDynmslToken] = useState('');
  ...
  const [orderBy, setOrderBy] = useState<'createdAt' | 'updatedAt' | 'targetEmail'>('targetEmail');
  ...
  const [exportFields, setExportFields] = useState<string[]>(['targetEmail', 'share']);

  const availableFields = [
    { key: 'targetEmail', label: '邮箱' },
    { key: 'password', label: '密码' },
    { key: 'share', label: '分享链接' },
    { key: 'shareType', label: '分享类型' },
    { key: 'expiresAt', label: '到期时间' },
    { key: 'remainingChecks', label: '剩余次数' },
  ...
  const handleExport = () => {
    ...
    window.open(`/api/export/emails?${qs.toString()}`, '_blank');
  };
  ...
  useEffect(() => {
    ...
    setDynmslToken(localStorage.getItem('dynmslApiToken') || '');
  }, []);
  ...
  const [importDurationDays, setImportDurationDays] = useState(30);
  ...
  const submitImport = async () => {
    try {
      ...
      // 1. Check duplicates and parse emails on server
      ...
      const duplicates = Array.isArray(res.data?.duplicates) ? res.data.duplicates : [];
      
      if (duplicates.length > 0) {
          setImportSkipped(duplicates);
      }

      // Filter out duplicates
       const toProcess = accounts.filter((a: { email: string }) => !duplicates.includes(a.email));
       if (toProcess.length === 0) {
           if (duplicates.length > 0) setError('导入失败：所有邮箱均已存在');
           else setError('未发现有效邮箱');
           return;
       }

       // 2. Register accounts that have passwords (Local request via proxy)
       const toRegister = toProcess.filter((a: { email: string; password?: string }) => a.password);
      if (toRegister.length > 0) {
        if (!dynmslToken.trim()) {
            setError('发现需要注册的邮箱（带密码），请先填写 DYNMSL Token');
            return;
        }
        localStorage.setItem('dynmslApiToken', dynmslToken.trim());

        const proxyRes = await fetch('/api-proxy/addUser', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: dynmslToken.trim(),
          },
          body: JSON.stringify({ list: toRegister }),
        });

        const text = await proxyRes.text();
        let json: { code?: number | string; success?: boolean; message?: string; error?: string } | null = null;
        try {
        ...
        const apiSuccess = json?.success === true || apiCode === 0 || apiCode === 200 || apiCode === '0' || apiCode === '200';

        if (!proxyRes.ok || !apiSuccess) {
          const msg = json?.message || json?.error || text || `注册失败 (${proxyRes.status})`;
          setError(`外部接口注册失败: ${msg}`);
          return;
        }
      }
      ...
      setImportRaw('');
      showToast(`成功导入 ${toProcess.length} 个邮箱`);
      await load();
    } catch (e: unknown) {
  ...
  const submitGenerate = async () => {
    try {
      ...
      setError('');
      if (!dynmslToken.trim()) {
        setError('请先填写 DYNMSL Token（仅保存在本机浏览器）');
        return;
      }
      localStorage.setItem('dynmslApiToken', dynmslToken.trim());

      // 1. Generate list and check duplicates on server
      const res = await axios.post('/api/emails', {
        userId,
        mode: 'generate',
        count: genCount,
      ...
      if (duplicates.length > 0) {
          setError(`检测到重复邮箱：${duplicates.join('、')}`);
          // We can still proceed with non-duplicates if needed, but for generation it's better to fail or retry
          return;
      }
      ...
      // 2. Register on external API (Local request via proxy)
      const proxyRes = await fetch('/api-proxy/addUser', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      showToast(`成功注册并保存 ${list.length} 个邮箱`);
      await load();
    } catch (e: unknown) {
      if (axios.isAxiosError(e)) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded shadow p-6">
          ...
          <textarea
            ...
            placeholder="支持格式：邮箱----密码 或 邮箱:密码 或 邮箱<TAB>密码（仅支持 @dynmsl.com）"
            value={importRaw}
        ...
        <div className="bg-white rounded shadow p-6">
          ...
          <input
            ...
            placeholder="DYNMSL Token（仅保存在本机浏览器）"
            value={dynmslToken}
            onChange={(e) => setDynmslToken(e.target.value)}
            autoComplete="off"
          />
          ...
          <div className="text-xs text-gray-500 mt-2">注册请求由浏览器发起（走 /api-proxy），避免服务端被 Cloudflare 拦截</div>
        </div>
      </div>
      ...
      <div className="bg-white rounded shadow p-6">
          ...
          <div className="flex gap-3 items-center">
            ...
            <div className="flex items-center gap-2 border-l pl-3 ml-1">
              ...
              <div className="relative group">
                <button className="text-gray-800 text-sm border rounded px-2 py-1 hover:bg-gray-50">
                  选择字段 ({exportFields.length})
                </button>
                <div className="hidden group-hover:block absolute right-0 top-full mt-1 bg-white border rounded shadow-lg p-3 z-10 w-48">
                  <div className="text-xs font-bold mb-2 border-b pb-1">导出字段</div>
                  {availableFields.map(f => (
                    <label key={f.key} className="flex items-center gap-2 text-xs py-1 cursor-pointer hover:bg-gray-50">
                      <input 
                        type="checkbox" 
              ...
              <button className="bg-gray-800 text-white text-sm px-3 py-1 rounded hover:bg-black transition" onClick={handleExport}>
                导出
              </button>
            </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <div className="flex items-center gap-2">
            <textarea
              className="border rounded p-2 text-sm w-64 h-20"
              placeholder="批量搜索：一行一个（模糊匹配）"
              value={queryInput}
          ...
          <div className="flex flex-wrap items-center gap-2 text-sm">
            ...
            <select
              ...
              onChange={(e) => {
                const v = e.target.value;
                if (v === 'createdAt' || v === 'updatedAt' || v === 'targetEmail') setOrderBy(v);
              }}
              disabled={busy}
            >
              <option value="targetEmail">按邮箱</option>
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-gray-50">
                ...
                <th className="border p-2 text-left">序号</th>
            ...
            <tbody>
              {configs.map((cfg, idx) => {
                ...
                const shareKey = cfg.shareId || cfg._id;
                const href =
                  t === 'json'
                    ? `${baseUrl}/api/share/email/${shareKey}`
                    : `${baseUrl}/share/email/${shareKey}`;
      ...
      {toast && (
        <div className={`fixed top-4 right-4 px-6 py-3 rounded shadow-lg text-white transition-all transform duration-300 translate-y-0 ${toast.type === 'success' ? 'bg-green-600' : 'bg-red-600'} z-50`}>
          <div className="flex items-center gap-2">
            {toast.type === 'success' ? (
