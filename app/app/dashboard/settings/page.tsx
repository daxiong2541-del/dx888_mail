...
export default function SettingsPage() {
  ...
  const [dynmslWafBypassToken, setDynmslWafBypassToken] = useState('');
  const [dynmslWafBypassTokenSet, setDynmslWafBypassTokenSet] = useState(false);
  ...
  const load = async (uid: string) => {
    ...
    setDynmslWafBypassTokenSet(Boolean(res.data.dynmslWafBypassTokenSet));
  };
  ...
  const save = async () => {
    try {
      ...
      await axios.put('/api/settings', {
        ...
        dynmslWafBypassToken: dynmslWafBypassToken ? dynmslWafBypassToken : undefined,
      });
      setDynmslApiToken('');
      setDynmslWafBypassToken('');
      await load(userId);
      setOk('已保存');
    } catch (e: unknown) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded shadow p-6 space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            <div className="text-sm font-semibold">DYNMSL_WAF_BYPASS_TOKEN</div>
            <div className="text-xs text-gray-600">当前状态：{dynmslWafBypassTokenSet ? '已设置' : '未设置'}</div>
          </div>
          <input
            className="border rounded p-2 w-full"
            type="password"
            value={dynmslWafBypassToken}
            onChange={(e) => setDynmslWafBypassToken(e.target.value)}
            placeholder={dynmslWafBypassTokenSet ? '输入新 bypass token 将覆盖旧 token' : '输入 bypass token'}
            disabled={busy}
          />
          <div className="text-xs text-gray-500 mt-1">当上游出现 Cloudflare 验证时需要配置该 token。</div>
        </div>
