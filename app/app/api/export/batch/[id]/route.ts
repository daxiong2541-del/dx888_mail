...
export async function GET(
  ...
  const cfgMap = new Map<
    string,
    { expiresAt?: string; maxCount?: number; receivedCount?: number; shareType?: string; shareId?: string }
  >();
  for (const cfg of configs) {
    cfgMap.set(String(cfg._id), {
      ...
      shareId: String((cfg as unknown as { shareId?: string }).shareId || ''),
    });
  }
  ...
  for (const acc of task.generatedAccounts || []) {
    ...
    const shareKey = info?.shareId || acc.emailConfigId;
    const share =
      shareKey
        ? shareType === 'json'
          ? baseUrl
            ? `${baseUrl}/api/share/email/${shareKey}`
            : `/api/share/email/${shareKey}`
          : baseUrl
            ? `${baseUrl}/share/email/${shareKey}`
            : `/share/email/${shareKey}`
        : '';
    rows.push(
      [
